const BLOCKLIST = {
    ids: [],
    names: ["Soundboard", "[!] COMMENTS", "Geometry Dash Lite (REMAKE)", "[!] SUGGEST GAMES .gg/D4c9VFYWyU", "Five Nights at Epstein's", "Five Nights at Last Breath"]
};

const EXTERNAL_TAB_LIST = [
    "[!] SUGGEST GAMES • Tiktok",
    "Upvote On UBGHUB!",
    "NikeHub Discord",
];

const POSITIONING_RULES = [
    { name: "[!] SUGGEST GAMES • Tiktok", position: 0 },
    { name: "Upvote On UBGHUB!", position: 1 }
];
...
// NEW: Second custom zones file URL
const customZonesURL2 = "/customzones2.json";
...
// NEW: Loader function for second custom zones file
async function loadCustomZonesFile2() {
    try {
        const response = await fetch(customZonesURL2 + "?t=" + Date.now());
        if (!response.ok) throw new Error('File not found');
        const data = await response.json();
        console.log(`Loaded ${data.length} zones from ${customZonesURL2}`);
        return data.map(zone => ({ zone, source: CDN_SOURCES[0] }));
    } catch (error) {
        console.warn('No customzones2.json file found:', error.message);
        return [];
    }
}
...
async function listZones() {
    try {
        const loadPromises = [
            ...
            loadCustomZonesFile(),
            loadCustomZonesFile2(), // NEW: Added second custom zones loader
...
function sortZones() {
    ...
    POSITIONING_RULES.forEach(rule => {
        const index = sortedZones.findIndex(z => z.name === rule.name);
        if (index !== -1) {
            const [item] = sortedZones.splice(index, 1);
            sortedZones.splice(rule.position, 0, item);
        }
    });
...
function openZone(file) {
    if (EXTERNAL_TAB_LIST.includes(file.name)) {
...
function stealthAboutBlank() {
    ...
    win.document.write(`
        ...
        <head>
            <meta charset="UTF-8">
            <title>clean</title>
            <style>body{background:#000;color:#0f0;font-family:monospace;padding:1rem;}</style>
        </head>
        <body>blank tab ready</body>
        </html>
    `);
    win.document.close();
}

function stealthAboutBlankCurrentPage() {
    const win = window.open('about:blank', '_blank');
    if (!win) return;
    ...
    console.log('[stealth] current page cloned to about:blank');
}

function createStealthBlob() {
    const textarea = document.getElementById('blobInput');
    let content = textarea?.value.trim() || '';

    if (!content && zoneFrame && zoneFrame.contentDocument) {
        try {
            content = zoneFrame.contentDocument.documentElement.outerHTML;
        } catch (e) {
            console.warn('couldnt read iframe content', e);
        }
    }

    if (!content) {
        alert('no content. open a zone first or paste something.');
        return;
    }

    let type = 'text/html';
    if (content.includes('function') || content.match(/const |let |var /)) {
    ...
    if (win) {
        console.log(`[stealth] blob opened: ${url}`);
        setTimeout(() => URL.revokeObjectURL(url), 600000);
    }
}

function createBlobCurrentPage() {
    const fullHtml = document.documentElement.outerHTML;
    ...
    if (win) {
        ...
        console.log(`[stealth] current page blobbed → ${url}`);
...
function openSettings() {
    ...
    document.getElementById('popupBody').innerHTML = `
        <div class="setting-item">
            <h3 style="color: var(--primary-hover); margin-bottom: 1rem; padding-bottom: 0.5rem; border-bottom: 1px solid var(--card-border);">Settings</h3>
        ...
        <div class="setting-item" style="margin-top: 1.5rem;">
            <button class="btn btn-primary" onclick="showContact()" style="width: 100%;">
                ...
                <button class="btn btn-primary" onclick="stealthAboutBlankCurrentPage()" style="width: 100%;">
                    <i class="fas fa-clone"></i> About:Blank
                </button>
            </div>
